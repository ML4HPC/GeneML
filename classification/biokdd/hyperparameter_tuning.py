    ...
    def objective(trial):
        ...
        for train_idx, val_idx in inner_split.split(X_train_val, y_train_val):
            ...
            if model_name in ['mlp','cnn']:
                ...
                clf.fit(X_fit, y_fit, eval_set=[(X_eval, y_eval)], eval_metric='auc')
...
def tune_train_evaluate_model(embeddings_combined, demographics, labels, model_name, method):
    ...
    for fold, (train_val_idx, test_idx) in enumerate(
        ...
        if model_name == 'mlp':
            study = optuna.create_study(direction='maximize', sampler=optuna.samplers.TPESampler(seed=98), pruner=optuna.pruners.HyperbandPruner(min_resource=1, max_resource=200, reduction_factor=2))
            study.optimize(objective, n_trials=15)  # 150 in total
        elif model_name == 'cnn':
            ...
            study.optimize(objective, n_trials=5)  # 50 in total
        else:
            ...
            study.optimize(objective, n_trials=30)  # 300 in total
        ...
        trials_df.to_csv(f'optuna_results/{model_name}/{method}/fold_{fold}_trial_info.csv', index=False)
    ...
    best_trials_df.to_csv(f'optuna_results/{model_name}/{method}/best_trials_info.csv', index=False)
